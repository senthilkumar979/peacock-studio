...
export const LandingSectionShell = ({
  ...
  <section id={id} className={TONE_CLASS[tone]}>
    <div className="landing-section-inner">
      ...
      >
        <p className={`text-xs font-semibold uppercase tracking-[0.16em] ${EYEBROW_CLASS[tone]}`}>
          {eyebrow}
        </p>
        <h2 className="mt-3 text-3xl font-bold tracking-tight sm:text-4xl">{title}</h2>
        {description ? (
          <p className={`mt-4 text-base leading-relaxed ${DESCRIPTION_CLASS[tone]}`}>{description}</p>
        ) : null}
      </motion.header>
