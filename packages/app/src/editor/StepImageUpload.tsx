...
const STEP_IMAGE_INFO_TOOLTIP =
  "JPEG, JPG, PNG, or SVG. Max 1 MB — larger raster images are compressed automatically. Replaces the captured screenshot for this step.";
...
export const StepImageUpload = ({ step }: StepImageUploadProps) => {
  ...
  const setStepCustomScreenshot = useFlowStore(
    (state) => state.setStepCustomScreenshot,
  );
    ...
    <div className="flex flex-col gap-2 text-sm">
      <div className="flex justify-between">
        <div className="flex items-center gap-1">
          ...
          <ActionTooltip label={STEP_IMAGE_INFO_TOOLTIP} wide>
            <button
              type="button"
              className="inline-flex rounded-full p-0.5 text-slate-400 transition hover:text-peacock-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-peacock-500 focus-visible:ring-offset-1"
              aria-label="About step image upload"
            >
      ...
      {hasCustomImage && (
        <p className="text-xs text-emerald-700">
          Using a custom image for this step.
        </p>
      )}
