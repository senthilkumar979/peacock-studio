...
9. **Secrets checklist** — Vercel: `VITE_TURNSTILE_SITE_KEY`. Supabase function secrets: `TURNSTILE_SECRET`, `APP_ORIGIN` (exact SPA origin for CORS), `RESEND_*`, `SUPER_ADMIN_EMAILS` (comma-separated platform super-admin emails for `/super-admin`), `POSTHOG_PERSONAL_API_KEY` (Query Read key for Acquisition tab; optional `POSTHOG_PROJECT_ID`, `POSTHOG_HOST`). Redeploy Edge Functions after secret changes: `send-org-invite`, `resolve-share`, `platform-admin`.
